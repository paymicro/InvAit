...
public sealed class ContextPublisher : IDisposable
{
    ...
    private VsContext _lastContext;
    private DateTime _lastUpdate = DateTime.MinValue;
    private const int _throttleMs = 500;

    private ContextPublisher(DTE2 dte, WebView2 webView)
    {
    ...
    public static async Task<ContextPublisher> CreateAsync(DTE2 dte, WebView2 webView)
    {
        var publisher = new ContextPublisher(dte, webView);
    ...
    private async Task UpdateContextAsync()
    {
        ...
        try
        {
            ...
            {
                context.SolutionFiles = await SolutionStructure.BuildStructureAsync();
                context.SolutionProjects = await SolutionStructure.GetProjectPathsAsync();
    ...
    private async Task SendContextUpdateAsync(VsContext context)
    {
