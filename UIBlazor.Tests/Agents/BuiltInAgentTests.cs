...
public class BuiltInAgentTests
{
    ...
    public BuiltInAgentTests()
    {
        ...
        _agent = new BuiltInAgent(_vsBridgeMock.Object, Mock.Of<ISkillService>(), Mock.Of<IInternalExecutor>(), Mock.Of<IContextService>());
    }
    ...
    public void Tools_AreRegistered()
    {
        ...
        Assert.Contains(_agent.Tools, t => t.Name == BuiltInToolEnum.EditFiles);
    }
    ...
    {
        // Arrange
        var tool = _agent.Tools.First(t => t.Name == BuiltInToolEnum.EditFiles);
        var args = JsonSerializer.Serialize(new Dictionary<string, object> { { "path", "test.cs" } });
        ...
        // Assert
        _vsBridgeMock.Verify(b => b.ExecuteToolAsync(BuiltInToolEnum.EditFiles, args, CancellationToken.None), Times.Once);
    }
