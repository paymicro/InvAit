...
- `find_declarations` and `find_references` are not yet implemented (require language server integration).
- MCP servers run in-process via the official TypeScript MCP SDK (no external .NET host needed).
- Single workspace folder supported.
